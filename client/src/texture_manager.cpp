...
void TextureManager::load_tile_textures(
    ...
    const std::string objects_dir = ARGENTUM_SHARE_PATH "/common/assets/maps/objets/";
    ...
    if (std::filesystem::exists(objects_dir)) {
        for (const auto& entry: std::filesystem::directory_iterator(objects_dir)) {
            if (entry.path().extension() != ".png")
                continue;
            const std::string id = entry.path().stem().string();
            const std::string detail_alias = "tile_det_" + id;
            if (!textures_aux.contains(detail_alias)) {
                load_texture(textures_aux, detail_alias, entry.path().string());
            }
        }
    }
}
