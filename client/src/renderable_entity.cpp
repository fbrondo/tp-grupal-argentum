...
void RenderableEntity::move_to(int target_tile_x, int target_tile_y, Direction dir) {
    ...
    if (tile_x != target_tile_x || tile_y != target_tile_y) {
        ...
        // Si el servidor nos mandó la misma baldosa pero otra dirección, solo giramos quietos (idle)
        is_moving = false;
        anim_state.current_anim_id = "body_" + std::to_string(body_id) + "_idle_" + std::to_string(dir);
    }
}
...
void RenderableEntity::update(float dt) {
    ...
    if (current_pixel_x != target_pixel_x || current_pixel_y != target_pixel_y) {
        ...
        if (is_moving) {
            ...
            anim_state.current_anim_id = "body_" + std::to_string(body_id) + "_idle_" + std::to_string(current_dir);
        }
    }
}

void RenderableEntity::render_with_camera(SDL2pp::Renderer& renderer, TextureManager& texture_manager,
                                          int cam_x, int cam_y, int offset_x, int offset_y) {
    if (anim_state.current_anim_id.empty()) return;
    ...
    try {
        ...
        SDL2pp::Texture& body_texture = const_cast<SDL2pp::Texture&>(texture_manager.get_texture(body_tex_key));
        ...
        if (head_id != 0) {
            ...
            SDL2pp::Texture& head_texture = const_cast<SDL2pp::Texture&>(texture_manager.get_texture(head_tex_key));
            ...
            renderer.Copy(head_texture, SDL2pp::Optional<SDL2pp::Rect>(), SDL2pp::Optional<SDL2pp::Rect>(SDL2pp::Rect(dst_head)));
        }
    } catch (const std::exception& e) {
        // Capturamos cualquier error por si el TextureManager no encuentra un ID (buena práctica para evitar crashes)
