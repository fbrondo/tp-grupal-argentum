...
private:
    ...
    std::unique_ptr<Creature> createCreature(const Id& id, TypeNPC type, const Pose& pose, const NpcAttributes& attrib);
    std::unique_ptr<CitizenNPC> createCitizenNpc(const std::string& name_npc, Bank& bank);

    std::tuple<TypeNPC, NpcAttributes, Pose> prepareNpcSpawn(const Id& zone_id, const HostileRegion& region, const size_t& index_random);
    std::tuple<TypeNPC, Pose> prepareCitizenNpcSpawn(const Id& zone_id, const SafeRegion &region, const size_t& index);
...
public:
    ...
    void spawnCitizenNpcZones(Id& next_id, std::map<Id, std::unique_ptr<CitizenNPC>>& citizen_npcs, Bank& bank);
