...
SpawnManager::SpawnManager(const GameConfig& conf, World& world_):
    gen(std::random_device{}()), world(world_), items(conf.items),
    conf_creatures(conf.creatures), conf_citizens(conf.npcs) {
...
std::map<TypeItem, Item*> SpawnManager::items_store_citizen(const std::string& name_npc) {
    ...
    for (const auto& type_item : items_store) {
        items_.emplace(type_item,this->items.at(type_item).get());
    }
    return items_;
}

std::vector<ItemInstance> SpawnManager::items_drop_creature() {
...
NpcAttributes SpawnManager::attributesToCreature(const CreatureConfig& creature) {
    ...
    std::uniform_int_distribution<uint16_t> distr_lervel(creature.minimal_level, creature.maximun_level);
    attrib.difficulty_level = distr_lervel(this->gen);
    return attrib;
}

std::tuple<TypeNPC, NpcAttributes, Pose> SpawnManager::prepareNpcSpawn(const Id& zone_id, const HostileRegion &region, const size_t& index_random) {
    ...
    for (const auto &zone: hostile_zones | std::views::values) {
        ...
        if ( count < respawn_threshold) {
            ...
            while (count < region->max_creatures) {
                const Id id = next_id++;
                auto [type, attrib, pose] = this->prepareNpcSpawn(zone.id, *region, distr_npc(this->gen));
...
std::tuple<TypeNPC, Pose> SpawnManager::prepareCitizenNpcSpawn(const Id& zone_id, const SafeRegion &region, const size_t& index) {
    const auto& name_npc = region.npc_types[index];;
    const auto& type_npc = this->conf_citizens.at(name_npc).type;
    const auto position_spawn = this->world.calculatePositionRandom(zone_id);
    Pose pose_spawn(position_spawn, DOWN);
...
void SpawnManager::spawnCitizenNpcZones(Id& next_id, std::map<Id, std::unique_ptr<CitizenNPC>>& citizen_npcs, Bank& bank) {
    ...
    for (const auto &zone: safe_zones | std::views::values) {
        const auto region = this->safe_regions.at(zone.region);
        for (size_t j = 0; const auto& name_npc: region->npc_types) {
            ...
            for (size_t i = 0; i < numbers_npc; i++) {
                auto [type, pose] = this->prepareCitizenNpcSpawn(zone.id, *region, j);
...
void SpawnManager::spawnTreasuresZones() {
    ...
    for (const auto &zone: hostile_zones | std::views::values) {
        ...
        if (region->min_treasure.has_value() && region->max_treasure.has_value() && count == 0) {
            std::uniform_int_distribution dist(region->min_treasure.value(), region->max_treasure.value());
