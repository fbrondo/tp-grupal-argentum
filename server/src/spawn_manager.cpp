...
std::unique_ptr<Creature> SpawnManager::createCreature(const Id& id, const std::string& name,
                                                       TypeNPC type, const Pose& pose,
                                                       const NpcAttributes& attrib) {
    std::vector<ItemInstance> items_ = this->items_drop_creature();
    return std::make_unique<Creature>(id, name, type, pose, attrib, std::move(items_));
}
