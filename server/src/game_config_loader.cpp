...
Statistics GameConfigLoader::loadTableStatistics(Table_array* stats_array) {
    ...
    if (stats_array && !stats_array->empty()) {
        ...
        stats.intelligense = static_cast<uint16_t>(stats_table["intelligence"].value_or(0)); // Ojo con la "s" en tu archivo
        stats.constitution = static_cast<uint16_t>(stats_table["constitution"].value_or(0)); // Ojo con la "i" intermedia
...
void GameConfigLoader::loadRaces(std::unordered_map<TypeRace, Race>& info_races) {
   try {
        ...
        for (auto&& node : *races_array) {
            ...
            Race race = Race(type, std::move(name), life_factor, recovery_factor, mana_factor, std::move(stats));
...
void GameConfigLoader::loadClases(std::unordered_map<TypeClase, Clase>& info_clases) {
    try {
        ...
        for (auto&& node : *clases_array) {
            ...
            auto meditation_factor = static_cast<uint16_t>(clase_table["metitation_factor"].value_or(1));
            ...
            Statistics stats = this->loadTableStatistics(stats_array);
            Clase clase = Clase(type, std::move(name), hp_factor, meditation_factor, mana_factor, std::move(stats));
...
const FileData GameConfigLoader::getFilesData() {
    return this->data;
}
    ...
    for (const auto& item_node: *items_array) {
        ...
        if(classif == ITEM_ATTACK) {
            if(descp == "MELEE_WEAPON") {
                info_items[type] = std::make_unique<Weapon>(type, body, classif, std::move(name), sell_price, purch_price, min_dam, max_dam);
            } else if(descp == "MAGICAL") {
                info_items[type] = std::make_unique<MagicWeapon>(type, body, classif, std::move(name), sell_price, purch_price, min_dam, max_dam, mana_cost, range);
            } else if(descp == "RANGED_WEAPON") {
                info_items[type] = std::make_unique<RangedWeapon>(type, body, classif, std::move(name), sell_price, purch_price, min_dam, max_dam, range);
            }   
        } else if(classif == ITEM_DEFENSIVE) {
            info_items[type] = std::make_unique<Defense>(type, body, classif, std::move(name), sell_price, purch_price, min_def, max_def);
        } else if(classif == ITEM_HEALING) {
             if(descp == "POTION") {
                info_items[type] = std::make_unique<Item>(type, body, classif, std::move(name), sell_price, purch_price);
            } else if(descp == "MAGICAL") {
                info_items[type] = std::make_unique<ObjectMagic>(type, body, classif, std::move(name), sell_price, purch_price, mana_cost, range);
            }
        }
    }
}
...
GameConfig GameConfigLoader::getdGameConfiguration()  {
    try {
        ...
        std::string mssgErr = "Error en loadGameConfiguration -- No se pudo parsear el archivo TOML";
        mssgErr += err.description();
        throw std::runtime_error(mssgErr);
    }
}
